...
import SidebarLinkGroup from "./Sidebargroupform";
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  useEffect(() => {
    ...
    if (sidebarExpanded) {
      document.body.classList.add("sidebar-expanded"); // More efficient way to toggle class
    } else {
      document.body.classList.remove("sidebar-expanded"); // More efficient way to toggle class
    }
  }, [sidebarExpanded]);
  ...
  return (
    <>
      <aside
        ref={sidebar}
        className={`absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col   bg-gray-800 overflow-y-hidden  duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 transition-transform ${
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5 ">
          ...
          >
           <svg fill="#ffffff" height="25px" width="28px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 476.213 476.213" xml:space="preserve" transform="rotate(0)" stroke="#ffffff"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <polygon points="476.213,223.107 57.427,223.107 151.82,128.713 130.607,107.5 0,238.106 130.607,368.714 151.82,347.5 57.427,253.107 476.213,253.107 "></polygon> </g></svg>
          </button>
        </div>
        {/* <!-- SIDEBAR HEADER --> */}

        <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear  bg-gray-800">
          {/* <!-- Sidebar Menu --> */}
          <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
            {/* <!-- Menu Group --> */}
            <div>
              <h3 className="mb-4  text-white ml-4 text-sm font-semibold ">
                MENU
              </h3>
                  ...
                  {(handleClick, open) => {
                    return (
                      <React.Fragment>
                        <NavLink
                          to="/"
                          className={`group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-white duration-300 ease-in-out hover:bg-graydark  ${
                            (pathname === "/" ||
            ...
            <div className="">
              <NavLink
                to="/auth/signup"
                className={({ isActive }) =>
                  "group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-white duration-300 ease-in-out hover:text-red-600 " +
                  (isActive && "!text-white")
                }
              >
                Logout
